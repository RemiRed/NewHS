...
public class ClickStateManager : MonoBehaviour
{
    ...
    void Start()
    {
        ActivePlayer = GameObject.FindGameObjectWithTag("Player").GetComponent<HeroScript>();
        combatManager = GameManager.Instance.GameMode.GetComponent<CombatManager>();
        ...
        else if (AbilityToResolve.Ability == Ability.None)
        {
            ...
            else if (!SpellToResolve)
            {
                if (!ActiveCard && clickedCard.Hero == ActivePlayer)
                {
                    ...
                    returnPosition = clickedCard.transform.position;
    ...
    public void ReturnToOriginalPosition(CardHand card)
    {
        card.transform.position = returnPosition;
    }
}